...
function NearbyPeople(props) {
    ...
    return (
        <Box>
            {people ? people.map(({ firstName, lastName, profilePic, userName }, index) => (
                <Box sx={{ display: 'block', height: '3em', p: 2, mt: 1 }}>
                    <Box sx={{ display: 'flex', float: 'left', left: 0 }}>
                        <ProfilePicHolder src={profilePic} width={38} height={38} />
                        <Box sx={{ml:3,width:'9em'}}>
                            <Box sx={{ fontWeight: 600, fontSize: '15px', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap', ':hover': { textDecorationLine: 'underline' } }} key={index}><Link href={'/user/' + userName}>{firstName + ' ' + lastName}</Link></Box>
                            <Box sx={{fontSize:'12px'}}>Nearby</Box>
                        </Box>
                    </Box>

                    <Box sx={{ float: 'right', right: 0 }}>
                        <Button onClick={()=>{alert('WORK IN PROGRESS')}} sx={{ borderRadius: '2em',fontWeight:600, height: '36px',justifyContent:'center',alignItems:'center', display: 'flex', fontSize: '14px', float: 'right' }}>{add}ADD</Button>
                    </Box>
                </Box>
            )) : null}
        </Box>
